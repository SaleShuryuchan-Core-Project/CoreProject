...
const PriceCheck = () => {
  ...
  const [selectedMode, setSelectedMode] = useState(null); // 사용자가 선택한 모델을 저장하는 상태
  ...
  // 모델 선택 시 용량 리스트 필터링
  useEffect(() => {
    if (selectedManufacturer && selectedSeries && selectedMode && rawData.length > 0) {
      const filteredVoumes = rawData
        .filter(
          (item) =>
            item.제조사 === selectedManufacturer &&
            item.시리즈 === selectedSeries &&
            item.모델 === selectedMode
        )
        .map((item) => item.용량)
        .filter((v, i, a) => a.indexOf(v) === i);
      setVoumeList(filteredVoumes);
    }
  }, [selectedMode, selectedManufacturer, selectedSeries, rawData]);
    ...
    <div className='priceCheckRoot'>
      <section className='priceCheckSection'>
        ...
        <div className='priceCheckListWrapper'>
              ...
              <div className='priceCheckStepList'>
                <div className='priceCheckTimeLine'>
                  <div className='priceCheckTimeLineContent'>
                    ...
                    {selectedMode && ( // 모델을 클릭했을때 h4태그 옆에 선택한 모델 표시
                      <p className='priceCheckTimeLineText'>
                        {selectedMode}
                      </p>
                    )}
                  ...
                  <ul className='priceCheckList'>
                    {modelList.map((model, index) => (
                      <li
                        key={index}
                        className='priceCheckListItem itemBox'
                        onClick={(e) => { e.stopPropagation(); handleModeClick(model); }}
                      >
                        {model}
                      </li>
                    ))}
          ...
          {/* 용량 선택 */}
          {currentStep >= 4 && selectedMode && voumeList.length > 0 && (
          ...
          {/* 시세조회 버튼 */}
          {selectedVoume && rawData.length > 0 && (
